...
public class TransactionHistory extends JFrame implements ActionListener, MouseListener{
    ...
    public TransactionHistory(String username, String name, Statement statement, int role){
        this.setTitle("Transaction History of -" + name);
        ...
        transactionLabel.setFont(new Font("century gothic", Font.PLAIN, 24));
        transactionLabel.setBounds(275, 5, 300, 40);
        transactionLabel.setForeground(Color.decode("#004d99"));
        ...
        try{
            ...
            for(index=0;dataResultSet.next();index++){
                ...
                for(int i=0;i<resultSetMetaData.getColumnCount();i++){
                    ...
                    if(i==1)
                        payment+=Double.parseDouble(dataResultSet.getString(column.get(i)));
                    else if(i==2)
                        marketCost+=Double.parseDouble(dataResultSet.getString(column.get(i)));
                }
                data.add(row[index]);
            }
            row[index] = new Vector<String>();
            row[index].add("Total");
            row[index].add(String.valueOf(payment));
            row[index].add(String.valueOf(marketCost));
            data.add(row[index]);
